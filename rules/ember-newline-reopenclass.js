...
module.exports = function(context) {
	...
	**/
	...
	function isReopenBlock(node) {
		...
		if (calleeType === "MemberExpression" && propertyName === "reopenClass") {
				return true;
		}
	...
	function checkForNewLineAroundBlock(node) {
		...
		if (isReopenBlock(node)) {
			...
			if (!isNewLineBefore(code, startLineNumber) && !isNewLineAfter(code, endLineNumber)) {
				...
				context.report(node, "New Line is required above and below reopenClass block.");
			} else if (!isNewLineBefore(code, startLineNumber)) {

				context.report(node, "New line is required above the reopenClass block.");
			} else if (!isNewLineAfter(code, endLineNumber)) {

				context.report(node, node.loc.end, "New line is required below the reopenClass block.");
			}
		}
